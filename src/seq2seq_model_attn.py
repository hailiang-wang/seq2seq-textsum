...
class Seq2SeqModel(object):
  ...
  def step(self, session, encoder_inputs, decoder_inputs, target_weights,
    ...
    if not forward_only:
      ...
      #output_feed.append(self.attn_mask)
    
    # Attention Weights: Fetching the attention weights matrix attn_matrix
    # self.attn_masks: A list with size bucket_num
    # self.attn_masks[bucket_id]:  A list of Tensor with shape [batch_size, encoder_length], list size decoder_length Ty
    print ('Fetching attention masks values')
    ty = len(self.attn_masks[bucket_id]) # output decoder length
    tx = self.attn_masks[bucket_id][0].get_shape()[1]  # input encoder length [batch_size, encoder_length]
    print ("Encoder length Tx value is %d" % tx)
    print ("Decoder length Ty value is %d" % ty)
    
    # Evaluate attention weights
    attn_out = session.run(self.attn_masks[bucket_id], input_feed)
    attn_matrix = np.zeros((ty,tx))  # shape ty * tx, aij the ith decoder attention on jth encoder
    for i in range(len(attn_out)):
      attn_vector = attn_out[i][0,] # shape [1, Tx], get a slice from array shape [batch_size, Tx]
      attn_matrix[i,] = attn_vector
    
    # Evaluate outputs values
    outputs = session.run(output_feed, input_feed)
    
    if not forward_only:
      return outputs[1], outputs[2], None, attn_matrix  # Gradient norm, loss, no outputs.
    else:
      return None, outputs[0], outputs[1:], attn_matrix  # No gradient norm, loss, outputs.
