    ...
    with codecs.open(data_path, "rb") as fi:
        for line in fi.readlines():
            ...
            if len(entities) == 2:
                ...
                target = entities[1]
                source_ids = [
                    int(x) for x in sentence_to_token_ids(
                        source, vocab)]
                target_ids = [
                    int(x) for x in sentence_to_token_ids(
                        target, vocab)]
...
def train():
    ...
    with tf.Session() as sess:
        ...
        train_set = read_chat_data(
            data_path, vocab_path, FLAGS.max_train_data_size)
        dev_set = read_chat_data(
            dev_data, vocab_path, FLAGS.max_train_data_size)
            ...
            if current_step % FLAGS.steps_per_checkpoint == 0:
                ...
                for bucket_id in xrange(len(_buckets)):
                    ...
                    _, eval_loss, _ = model.step(
                        sess, encoder_inputs, decoder_inputs, target_weights, bucket_id, True, beam_search)
                    eval_ppx = math.exp(
        ...
        if beam_search:
            ...
            while sentence:
                ...
                # If there is an EOS symbol in outputs, cut them at that point.
                    ...
                    rec = " ".join([tf.compat.as_str(rev_vocab[output])
            ...
            while sentence:
                ...
                print(" ".join([tf.compat.as_str(rev_vocab[output])
