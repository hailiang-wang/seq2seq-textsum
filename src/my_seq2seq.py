            ...
            outputs.append(
                tf.argmax(
                    nn_ops.xw_plus_b(
                        ...
                        output_projection[1]),
                    dimension=1))
    return outputs, state, tf.reshape(tf.concat(
        0, beam_path), [-1, beam_size]), tf.reshape(tf.concat(0, beam_symbols), [-1, beam_size])
...
def decode_model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
    ...
    with tf.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                               reuse=True if j > 0 else None):
                bucket_outputs, _, beam_path, beam_symbol = seq2seq(
