...
config = dict({
              "hanlp_host": "127.0.0.1",
              "hanlp_port": 3002
              })


def word_segment(utterance):
    '''
    segment words
    '''
    url = "http://%s:%d/tokenizer" % (config["hanlp_host"], config["hanlp_port"])
    headers = dict({
    ...
    try:
        ...
        if result["status"] == "success":
            for o in result["data"]:
                tags.append(o["nature"])
                if o["nature"].startswith("nr"):
                    words.append("TPERSON")
                else:
                    words.append(o["word"])
        else:
            raise "is_chat_query exception"
    except:
        print("seg error\n", utterance)
...
def preprocess_sohu_business_raw_txt():
    '''
    •   特殊字符：去除特殊字符，如：“「，」,￥,…”；
    •   括号内的内容：如表情符，【嘻嘻】，【哈哈】
    ...
    •   替换数字：TAG_NUMBER；
    '''
    file_path = os.path.join(curdir, os.path.pardir, "tmp", "business.sohu.com.raw.txt")
    with open(file_path, "r") as fin:
        for x in fin.readlines():
            ...
            if len(c.split(" ")) > 3 and len(t.split(" ")) > 3:
                append_line_to_file(c + "\n", os.path.join(curdir, os.path.pardir, "tmp", "business.sohu.com.content.txt"))
                append_line_to_file(t + "\n", os.path.join(curdir, os.path.pardir, "tmp", "business.sohu.com.title.txt"))
